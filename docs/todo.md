...
### Web Speech API 安定化
- [x] `speechSynthesis.getVoices()` の初回空配列に備え、`voiceschanged` を待って中国語音声を再選択する初期化フローを追加する。
- [x] 読み上げ開始・終了・キャンセル・エラーを一元管理し、ボタンや再生中UIが取り残されないようにする。
- [x] 連続タップ時は `cancel()` してから新しい発話を開始し、多重再生を防ぐ。
- [x] 非対応ブラウザでは読み上げボタンを無効化して代替メッセージを表示する。

---

## 優先度中（追加済み）

- [x] キャラ情報パネル（プロフィール・Lv別セリフ・顔写真切替）追加。アバター/名前タップで開く。
- [x] ステータス確認パネル（親密度/進捗/習得語数）追加。📊ボタンから開く。
...
仕様書：`中国語コンテンツ経営/CPO_プロダクト/開発状況/恋愛チャット風中国語学習アプリ/コンテキスト/これまでに考えたこと/26-06-24_HSKK往復練習＋AI言い回し提案_Codex仕様書.md`

- [x] HSKKフィードバックのLanguageModel出力言語をjaに固定（zh非対応バグ修正・2026-06-24）
- [ ] HSKK往復練習ループ（自由回答→褒め→「言ってみ」→2回目認識→ゆるい判定で褒め/再促し）
- [ ] 褒め・励まし・再促し文を陸の声でテンプレ化（ランダム出し・です/ます禁止）
- [ ] AI言い回し提案層（Nanoでユーザー中文をrephrase／ピンインはpinyin-pro等ライブラリ／失敗時はquestion.modelへフォールバック）
- [ ] ★先行スパイク：Nanoが実用的な中文rephraseを返すか検証 → lilian報告 → 本実装判断
- [ ] 名場面セリフのローカルTTS MP3化（上記「優先度高・音声アセット設計」と合流）
